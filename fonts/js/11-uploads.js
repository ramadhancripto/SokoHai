...
(function () { 'use strict';
    ...
    if (!window.SKH_PLACEHOLDER_IMG) {
        window.SKH_PLACEHOLDER_IMG = 'data:image/svg+xml;utf8,' + encodeURIComponent( '<svg xmlns="http://www.w3.org/2000/svg" width="500" height="500">' + '<rect width="500" height="500" fill="#e8f0f7"/>' + '<rect x="20" y="20" width="460" height="460" rx="28" fill="#f8fafc" stroke="#cbd5e1" stroke-width="2"/>' + '<circle cx="250" cy="205" r="78" fill="#00509d" opacity="0.92"/>' + '<path d="M215 190 h70 l-9 66 a8 8 0 0 1 -8 7 h-36 a8 8 0 0 1 -8 -7 z" fill="#f6c445"/>' + '<path d="M228 190 a22 22 0 0 1 44 0" fill="none" stroke="#0f172a" stroke-width="7" stroke-linecap="round"/>' + '<text x="250" y="330" font-family="Arial,Helvetica,sans-serif" font-size="34" font-weight="bold" fill="#0f172a" text-anchor="middle">SOKOHAI</text>' + '<text x="250" y="372" font-family="Arial,Helvetica,sans-serif" font-size="22" fill="#64748b" text-anchor="middle">Picha inakuja hivi karibuni</text>' + '</svg>');
    }
    ...
    window.skhUploadFromFile = async function (file, opts) {
        ...
        try {
            ...
            const res = await fetch( `https://api.cloudinary.com/v1_1/${window.SOKOHAI_UPLOADS.cloud}/${resource}/upload`,
                { method: 'POST', body: fd }
            );
    ...
    window.skhInitPhotoPicker = function (cfg) {
        ...
        function render() {
            ...
            list().forEach(function (f, i) {
                ...
                x.type = 'button';
                x.textContent = '\u2715';
                x.style.cssText = 'position:absolute;top:1px;right:1px;width:20px;height:20px;border:none;border-radius:50%;background:rgba(15,23,42,.85);color:#fff;font-size:13px;cursor:pointer;line-height:1;';

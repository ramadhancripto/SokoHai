...
(function () {
    ...
    window.openSellerProfile = async function (sellerUid, sellerName) {
        ...
        var logo = (seller && (seller.logo || seller.profileImage || seller.photoURL)) || ('https://ui-avatars.com/api/?name=' + encodeURIComponent(name) + '&background=03509d&color=ffffff');
    ...
    function profHeader() {
        var coverHtml = P.cover
            ? '<div style="height:120px;background:url(' + esc(P.cover) + ') center/cover no-repeat;border-radius:14px 14px 0 0;background-color:#e2e8f0;"></div>'
            : '<div style="height:120px;background:linear-gradient(135deg,#04162e,#03509d);border-radius:14px 14px 0 0;"></div>';
        var since = P.memberSince ? '<span style="font-size:11px;color:#64748b;">' + T('sst_member_since', 'Member since') + ' ' + esc(new Date(P.memberSince).getFullYear()) + '</span>' : '';
        var trustRow = P.fulfillment != null
            ? '<span style="font-size:11px;color:#0f172a;">' + T('sst_completion_rate', 'Completion rate') + ': <b>' + P.fulfillment + '%</b></span>' : '';
        var verifiedBadge = P.verified ? '<span style="font-size:10px;background:#dcfce7;color:#16a34a;padding:2px 8px;border-radius:10px;font-weight:800;">' + T('verified', 'Verified') + '</span>' : '';
        return `
        ${coverHtml}
        <div style="padding:0 16px 12px; background:white; border-radius:0 0 14px 14px; margin-bottom:10px; position:relative;">
            <div style="display:flex; justify-content:space-between; align-items:flex-end; margin-top:-34px;">
                <img src="${esc(P.logo)}" onerror="this.src='https://ui-avatars.com/api/?name=Store&background=03509d&color=ffffff'" style="width:68px;height:68px;border-radius:18px;border:3px solid white;background:#f1f5f9;object-fit:cover;box-shadow:0 4px 10px rgba(0,0,0,0.15);">
                <button id="spFollowBtn" onclick="window.skhProfileFollow()" class="follow-btn btn-not-following" style="margin-bottom:6px;">${T('eng_follow', 'Follow')}</button>
            </div>
            <div style="margin-top:8px;">
                <b style="font-size:17px;color:#0f172a;display:flex;align-items:center;gap:6px;flex-wrap:wrap;">${esc(P.name)} ${verifiedBadge}</b>
                <div style="font-size:12px;color:#475569;margin-top:2px;">${esc(P.loc)} · ${since}</div>
                <div style="display:flex;gap:14px;margin-top:8px;flex-wrap:wrap;font-size:12px;">
                    <span style="color:#f59e0b;font-weight:800;">${starSvg('#f59e0b')} ${esc(P.ratingTxt)}</span>
                    <span><b id="spFollowers">${P.followers}</b> ${T('sst_followers', 'Followers')}</span>
                    <span><b>${P.successOrders}</b> ${T('pm_completed_orders', 'Completed Orders')}</span>
                    ${trustRow}
                </div>
            </div>
        </div>`;
    ...
    function profTabs() {
        ...
        return `<div style="display:flex;gap:6px;padding:0 12px 10px;overflow-x:auto;">
            ${tabs.map(function (t) {
                ...
                return `<button class="skh-mytab" onclick="window.skhSellerProfileTab('${t[0]}', this)" style="flex:1;min-width:70px;padding:9px 6px;background:${active ? '#001122' : '#e2e8f0'};color:${active ? '#fff' : '#334155'};border:none;border-radius:10px;font-weight:800;font-size:11px;cursor:pointer;white-space:nowrap;">${t[1]}</button>`;
            }).join('')}
        </div>`;
    }
    ...
    function cardGrid(list) {
        ...
        return `<div class="feed-grid" style="padding:4px 12px 20px;grid-template-columns:repeat(auto-fill,minmax(140px,1fr));">
            ${list.map(function (p) {
                ...
                return `<div class="feed-card-box" onclick="openProduct('${js(p.id)}','products')">
                    <div class="feed-img-box"><img src="${img}" loading="lazy" onerror="this.src='https://ui-avatars.com/api/?name=Bidhaa&background=f1f5f9&color=64748b'"></div>
                    <div class="feed-info-box">
                        <b>${esc(p.title || T('products', 'Products'))}</b>
                        <span class="price" style="color:var(--terracotta);font-weight:900;">${tzs(p.price)}</span>
                        <div class="stats-row"><small>${p.views || 0} ${T('sst_views', 'views')}</small><small>${p.unitsSold || 0} ${T('sst_sold', 'sold')}</small></div>
                    </div>
                </div>`;
            }).join('')}
        </div>`;
    ...
    window.skhRenderSellerProfile = function () {
        ...
        if (!body || !P) return;
        var html = '';
        html += `<div style="position:sticky;top:0;z-index:20;background:#f4f7fa;padding:8px 12px;display:flex;justify-content:flex-end;">
            <button onclick="document.getElementById('sellerProfileModal').style.display='none'" style="background:#001122;border:none;color:white;width:32px;height:32px;border-radius:50%;font-size:16px;cursor:pointer;">&#10005;</button>
        </div>`;
        html += profHeader();
        html += profTabs();
        ...
        if (P_TAB === 'store') {
            html += `<div style="padding:0 12px 20px;">
                <div style="background:white;border:1px solid #e2e8f0;border-radius:14px;padding:14px;margin-bottom:10px;">
                    <b style="font-size:12px;color:#0f172a;display:block;margin-bottom:8px;text-transform:uppercase;">${T('sst_trust', 'Seller Trust')}</b>
                    <div style="font-size:12px;color:#334155;line-height:1.9;">
                        <div>${P.verified ? '<span style="color:#16a34a;font-weight:800;">' + T('sst_verified_seller', 'Verified Seller') + '</span>' : '<span style="color:#94a3b8;">' + T('sst_not_verified', 'Not verified yet') + '</span>'}</div>
                        <div>${T('pm_completed_orders', 'Completed Orders')}: <b>${P.successOrders}</b></div>
                ...
                <b style="font-size:12px;color:#0f172a;display:block;margin:6px 2px 8px;text-transform:uppercase;">${T('sst_featured', 'Featured')}</b>
            ...
            html += `<div style="padding:0 12px 12px;">
                <input id="spSearch" oninput="window.skhProfileSearch(this.value)" placeholder="${T('sst_search_ph', 'Search this store...')}" value="${esc(P_Q)}" style="width:100%;box-sizing:border-box;padding:11px 14px;border:1px solid #cbd5e1;border-radius:10px;font-size:13px;margin-bottom:8px;outline:none;">
                <div style="display:flex;gap:6px;margin-bottom:8px;overflow-x:auto;">
                    <select id="spCat" onchange="window.skhProfileCat(this.value)" style="flex:1;min-width:120px;padding:9px;border:1px solid #cbd5e1;border-radius:10px;font-size:12px;background:white;">
                    ...
                    <select id="spSort" onchange="window.skhProfileSort(this.value)" style="flex:1;min-width:110px;padding:9px;border:1px solid #cbd5e1;border-radius:10px;font-size:12px;background:white;">
                        <option value="newest" ${P_SORT === 'newest' ? 'selected' : ''}>${T('smp_newest', 'Newest')}</option>
                        <option value="best" ${P_SORT === 'best' ? 'selected' : ''}>${T('smp_best', 'Best selling')}</option>
                        <option value="deals" ${P_SORT === 'deals' ? 'selected' : ''}>${T('sst_tab_deals', 'Deals')}</option>
                    </select>
                </div>
                <div id="spProductsGrid"></div>
            </div>`;
            ...
            html += `<div style="padding:0 12px 20px;">
                ${reviews.length ? reviews.map(function (c) {
                    ...
                    return `<div style="background:white;border:1px solid #e2e8f0;border-radius:12px;padding:12px;margin-bottom:8px;">
                        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px;">
                            <b style="font-size:12px;color:#0f172a;">${esc(c.authorName || T('sst_customer', 'Customer'))}</b>
                            <span style="font-size:10px;color:#94a3b8;">${esc(new Date(c.timestamp).toLocaleDateString(window.SokoHaiLMS && window.SokoHaiLMS.lang === 'en' ? 'en-GB' : 'sw-TZ'))}</span>
                        </div>
                        <div style="margin-bottom:4px;">${stars}</div>
                        <p style="margin:0;font-size:13px;color:#334155;">${esc(c.text || c.comment)}</p>
                    </div>`;
            ...
            html += `<div style="padding:0 12px 20px;">
                <div style="background:white;border:1px solid #e2e8f0;border-radius:14px;padding:14px;">
                    <b style="font-size:12px;color:#0f172a;display:block;margin-bottom:8px;text-transform:uppercase;">${T('sst_about_store', 'About Store')}</b>
                    <p style="margin:0;font-size:13px;color:#334155;line-height:1.6;">${esc(P.about || T('sst_no_about', 'This seller has not written a store description yet.'))}</p>
                </div>
            </div>`;
        }

...
import {
  ...
  FIT_MODES,FOCAL_POINTS,ASPECT_RATIOS,
...
function shell(){
 ...
 <main class="skh-cs-layout">
   <aside class="skh-cs-left">
     <nav>
       ${['content','media','animation','templates','style','effects','cutout','uploads','advanced'].map(x=>`<button data-tab="${x}">${x}</button>`).join('')}
     </nav>
...
function renderLibrary(showVars=false){
  ...
  else if(activeTab==='uploads')box.innerHTML=`<h3>Uploads</h3><label class="cs-upload">Upload media<input id="csMediaFile" type="file" accept="image/*,video/*" hidden></label><button class="cs-wide" data-add="image-url">Use HTTPS media URL</button><p>Uses the existing SokoHai Cloudinary uploader. Originals are not stored in Firestore.</p>`;

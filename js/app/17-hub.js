...
window.renderSokoPayPieChart = function(prod, serv, trans) {
    ...
    window.safeCreateChart('spEscrowPieChart', {
        ...
        data: {
            ...
            datasets: [{
                data: values,
                backgroundColor: ['#18A982', '#4D91AA', '#D8B83A'],
                borderWidth: 0
            }]
        },
        options: {
    ...
    window.safeCreateChart('spDashboardWalletChart', {
        ...
        data: {
            ...
            datasets: [{
                ...
                backgroundColor: empty ? ['#e2e8f0'] : ['#18A982', '#4D91AA', '#D8B83A'],
                borderWidth: 0
            }]
        },
        options: {
...
window.spRenderGenResult = function(item) {
    ...
    el.innerHTML = `
        <div style="background:#f0f9ff; border:1px solid #bae6fd; border-radius:14px; padding:16px; margin-bottom:4px;"> <b style="font-size:13px; color:#0369a1; display:flex; align-items:center; gap:6px;"> <svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="#0369a1" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>
                ...
                ${item.qr ? `<div style="display:flex; flex-direction:column; gap:8px;"> <a href="${item.qr}" download="sokopay-${skh.skhEscape(item.code)}.png" style="text-align:center; padding:9px 14px; background:#526962; color:#fff; border-radius:10px; font-weight:800; font-size:12px; text-decoration:none;">&#11015; Pakua PNG</a> <button type="button" id="spCopyQrBtn" onclick="window.spCopyHaiPay(document.querySelector('#spGenResult img').src, 'spCopyQrBtn')" style="padding:9px 14px; background:#e2e8f0; color:#0f172a; border:none; border-radius:10px; font-weight:800; font-size:12px; cursor:pointer;">Nakili QR</button> </div>` : ''}

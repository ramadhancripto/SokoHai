...
window.openAnnouncementFormModal=function(editId){if(!((window.SOKOHAI_CLAIMS&&window.SOKOHAI_CLAIMS.isAdmin)||(skh.currentUser&&skh.currentUser.email===skh.MY_ADMIN_EMAIL))){alert('Admin authorization required.');return;}window.__editingAnnouncementId=editId||null;const existing=editId?(window.__sokohaiAnnouncementsCache||[]).find(a=>a.id===editId):null;let modal=document.getElementById('announcementFormModal');if(!modal){modal=document.createElement('div');modal.id='announcementFormModal';modal.className='overlay-menu';modal.style.cssText='z-index:9999;display:none;';document.body.appendChild(modal);}modal.innerHTML=window.skhAdFormHtml();const set=(id,v)=>{const e=document.getElementById(id);if(e)e.value=v||'';};set('annCreativeType',existing?.creativeType||'image_text');set('annBrand',existing?.brandName||'');set('annHeadline',existing?.headline||existing?.title||'');set('annText',existing?.description||existing?.text||'');set('annCta',existing?.ctaLabel||'');set('annLink',existing?.link||'');set('annImage',existing?.image||existing?.imageUrl||'');set('annVideo',existing?.videoUrl||'');set('annAudio',existing?.audioUrl||'');set('annLogo',existing?.logoUrl||'');set('annStartAt',existing?.startAt?existing.startAt.slice(0,16):'');set('annEndAt',existing?.endAt?existing.endAt.slice(0,16):'');set('annPriority',existing?.priority||0);set('annStatus',existing?(existing.status||((existing.active===false)?'draft':'published')):'draft');document.getElementById('annFormTitle').textContent=existing?'Edit Advertisement':'Create Advertisement';modal.style.display='flex';window.skhRenderAdminAdPreview();};

window.skhAdFormData=function(){const v=id=>(document.getElementById(id)?.value||'').trim();return{creativeType:v('annCreativeType'),brandName:v('annBrand'),headline:v('annHeadline'),description:v('annText'),text:v('annText'),ctaLabel:v('annCta'),link:v('annLink'),image:v('annImage'),imageUrl:v('annImage'),videoUrl:v('annVideo'),audioUrl:v('annAudio'),logoUrl:v('annLogo'),startAt:v('annStartAt')?new Date(v('annStartAt')).toISOString():'',endAt:v('annEndAt')?new Date(v('annEndAt')).toISOString():'',priority:Math.max(0,Number(v('annPriority'))||0),status:v('annStatus')||'draft',active:v('annStatus')==='published',archived:false};};
window.skhRenderAdminAdPreview=function(){const host=document.getElementById('annPreview');if(!host)return;const a=window.skhAdFormData(),video=a.creativeType.includes('video')&&a.videoUrl,image=a.imageUrl;let media=video?'<video muted playsinline controls poster="'+skh.skhEscape(image)+'" style="width:100%;height:180px;object-fit:cover;background:#101816"><source src="'+skh.skhEscape(a.videoUrl)+'"></video>':(image?'<img src="'+skh.skhEscape(image)+'" style="width:100%;height:180px;object-fit:cover" onerror="this.style.opacity=.2">':'<div style="height:180px;background:#EAF8F2;display:grid;place-items:center;color:#65757A">Creative preview</div>');host.innerHTML=media+'<div style="padding:14px;background:#fff"><small style="color:#39779B;font-weight:900">ADVERTISEMENT · '+skh.skhEscape(a.brandName||'Brand')+'</small><h3 style="margin:7px 0;color:#18352D">'+skh.skhEscape(a.headline||'Headline')+'</h3><p style="color:#65757A;font-size:12px">'+skh.skhEscape(a.description||'Short description')+'</p>'+(a.ctaLabel?'<span style="display:inline-block;background:#18A982;color:#fff;padding:8px 11px;border-radius:9px;font-weight:800;font-size:11px">'+skh.skhEscape(a.ctaLabel)+'</span>':'')+'</div>';const c=document.getElementById('annPreviewConfirmed');if(c)c.checked=false;};

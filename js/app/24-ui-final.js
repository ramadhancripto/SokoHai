...
/* [PERF 2026-09-21] Kadi hii huundwa mara moja tu — ukaguzi wa kila sekunde
   milele ni kazi bure. Koma baada ya majaribio 60 (dakika 1). */
(function(){ var __n = 0; var __iv = setInterval(function(){ try { document.getElementById('skhFriendlySectionCard')?.remove(); } catch(e){} if (++__n >= 60) { try { clearInterval(__iv); } catch(e2){} } }, 1000); })();
...
(function(){
  ...
  const LMS = {
    ...
    register(key, en, sw, status='draft'){
      ...
      /* [PERF 2026-09-21] register() iliandika localStorage kwa KILA key
         (32-i18n husajili keys 1200+ wakati wa startup → 1200+ sync writes
         zenye JSON inayokua O(n²) — jank kubwa kwenye simu ndogo).
         Sasa: cache kwenye memory + persist MARA MOJA (debounced). */
      try {
        this._customCache = this._customCache || {};
        this._customCache[key]={en,sw,status,updatedAt:new Date().toISOString()};
        if (this._customT) { try { clearTimeout(this._customT); } catch(e){} }
        this._customT = setTimeout(()=>{ try { this.flushCustom(); } catch(e){} }, 800);
      } catch(e){}
    },
    flushCustom(){
      try {
        if (!this._customCache) return;
        skh.localStorage.setItem('sokohai_lms_custom', JSON.stringify(this._customCache));
      } catch(e){}
    },
  ...
  LMS.loadCustom();
  /* [PERF 2026-09-21] Hakikisha custom translations zilizowekwa kwenye
     cache zinaandikwa hata mtumiaji akiondoka kabla ya debounce kuiva. */
  try {
    const __skhFlushLMS = ()=>{ try { LMS.flushCustom(); } catch(e){} };
    document.addEventListener('visibilitychange', ()=>{ if (document.hidden) __skhFlushLMS(); });
    window.addEventListener('pagehide', __skhFlushLMS);
  } catch(e){}
...
(function () {
    ...
    /* [PERF 2026-09-21] refreshSellFab huchambua overlays zote kwa
       getComputedStyle (~50-100 nodes) — 800ms milele ni kazi bure na
       inamaliza betri. 2000ms + ruka tab iliyofichwa inatosha kwa FAB. */
    function __skhFabTick() { try { if (!document.hidden) refreshSellFab(); } catch (e) {} }
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', function () { refreshSellFab(); setInterval(__skhFabTick, 2000); });
    } else {
        refreshSellFab();
        setInterval(__skhFabTick, 2000);
    }
})();
;

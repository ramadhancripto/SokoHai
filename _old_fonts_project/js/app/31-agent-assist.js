...
(function () { 'use strict';
    ...
    function injectBanner() {
        ...
        banner.setAttribute('style', 'position:fixed; top:0; left:0; right:0; z-index:9000; ' + 'display:flex; flex-wrap:wrap; align-items:center; justify-content:space-between; gap:10px; ' + 'padding:10px 14px; background:#1268A8; color:#fff; box-shadow:0 4px 16px rgba(0,0,0,0.25); font-family:inherit;');
    ...
    window.skhAssistHome = function () {
        ...
        if (!c) return;
        c.innerHTML = '<div style="max-width:640px; margin:0 auto;">' + '  <div style="background:linear-gradient(135deg,#1268A8,#0f172a); color:#fff; border-radius:20px; padding:24px; margin-bottom:18px;">' + '    <h2 style="margin:0 0 6px; font-weight:900;">Msaada wa Mwanachama</h2>' + '    <p style="margin:0; font-size:12px; opacity:0.85; line-height:1.5;">Sajili mwanachama asiye na simu, au msaidie aliyepo kuingia.</p>' + '  </div>' + '  <button onclick="window.skhAssistRegisterView()" style="width:100%; padding:18px; background:var(--gold); color:var(--primary-dark); border:none; border-radius:16px; font-weight:900; font-size:15px; cursor:pointer; margin-bottom:12px;">SAJILI MWANACHAMA MPYA (ASIYE NA SIMU)</button>' + '  <button onclick="window.skhAssistLoginView()" style="width:100%; padding:18px; background:#1268A8; color:#fff; border:none; border-radius:16px; font-weight:900; font-size:15px; cursor:pointer; margin-bottom:12px;">SAIDIA MWANACHAMA ALIYEPO (INGIA)</button>' + '  <div style="display:grid; grid-template-columns:1fr 1fr; gap:12px;">' + '    <button onclick="window.skhAssistMembersView()" style="padding:14px; background:#f8fafc; color:#0f172a; border:1px solid #cbd5e1; border-radius:14px; font-weight:bold; font-size:13px; cursor:pointer;">WANACHAMA WANGU</button>' + '    <button onclick="window.skhAssistActivityView()" style="padding:14px; background:#f8fafc; color:#0f172a; border:1px solid #cbd5e1; border-radius:14px; font-weight:bold; font-size:13px; cursor:pointer;">KUMBUKUMBU (AUDIT)</button>' +
  '  </div>' +
  ...
  '  <div style="display:none">' + '  </div>' + '  <button onclick="window.loadAgentDashboard()" style="width:100%; margin-top:12px; padding:12px; background:#e2e8f0; color:#475569; border:none; border-radius:12px; font-weight:bold; cursor:pointer;">RUDI KWENYE DAFTARI LA WAKALA</button>' + '</div>';
    ...
    window.skhAssistRegisterStep2 = function () {
        ...
        c.innerHTML = '<div style="max-width:480px; margin:0 auto; background:#fff; padding:24px; border-radius:20px; border:1.5px solid #1268A8;">' + '  <div style="text-align:center; margin-bottom:14px;">' + '    <div style="display:inline-block; background:#fffbeb; border:1px solid var(--gold); border-radius:12px; padding:10px 14px; font-size:12px; color:#92400e; font-weight:bold; margin-bottom:10px;">' + ' ' + escapeHtml(name) + ' aandike PIN yake mwenyewe' + '    </div>' + '  </div>' +
            pinFieldsHtml('assistReg') + '  <button id="btnAssistRegSubmit" onclick="window.skhAssistRegisterSubmit()" style="width:100%; padding:16px; background:var(--gold); color:var(--primary-dark); border:none; border-radius:14px; font-weight:900; font-size:14px; cursor:pointer;">MALIZA USAJILI</button>' + '  <button onclick="window.skhAssistRegisterView()" style="width:100%; margin-top:10px; padding:12px; background:#e2e8f0; color:#475569; border:none; border-radius:12px; font-weight:bold; cursor:pointer;">RUDI</button>' + '</div>';
    };
    ...
    window.skhAssistMembersView = async function () {
        ...
        try {
            ...
            var rows = members.length ? members.map(function (m) {
                ...
                return '' + '<div style="padding:14px; background:#f8fafc; border:1px solid #e2e8f0; border-radius:12px; display:flex; flex-wrap:wrap; gap:10px; justify-content:space-between; align-items:center;">' + '  <div style="min-width:180px;">' + '    <b style="font-size:14px; color:#0f172a;">' + escapeHtml(m.fullName) + '</b>' + '    <span style="display:block; font-size:13px; font-weight:900; color:#1268A8;">' + escapeHtml(m.memberCode) + '</span>' + '    <span style="display:block; font-size:13px; color:gray;">' + escapeHtml(m.region) + (m.district ? ' · ' + escapeHtml(m.district) : '') + '</span>' + '    <span style="display:block; font-size:12.5px; color:gray;">Status: <b style="color:' + statusColor + ';">' + escapeHtml(m.status) + '</b>' + '      · Mwisho kusaidiwa: ' + (m.lastAssistedAt ? new Date(m.lastAssistedAt).toLocaleString() : '—') + '</span>' + '  </div>' + '  <div style="display:flex; gap:6px; flex-wrap:wrap;">' + '    <button onclick="window.skhAssistLoginView(\'' + escapeHtml(m.memberCode) + '\')" style="padding:9px 12px; background:#1268A8; color:#fff; border:none; border-radius:8px; font-weight:bold; font-size:13px; cursor:pointer;">SAIDIA</button>' +
                (m.status === 'ACTIVE'
